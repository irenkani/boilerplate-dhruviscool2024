...
exports.registerInvite = exports.resetPassword = exports.sendResetPasswordEmail = exports.verifyAccount = exports.approve = exports.register = exports.logout = exports.login = void 0;
...
const login = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    (err, user, info) => {
        ...
        if (!user.verified) {
            next(apiError_1.default.unauthorized('Need to verify account by email'));
            return;
        }
...
const register = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!firstName || !lastName || !email || !password) {
        next(apiError_1.default.missingFields(['firstName', 'lastName', 'email', 'password']));
        return;
    }
    ...
    const existingUser = yield (0, user_service_1.getUserByEmail)(lowercaseEmail);
    if (existingUser) {
        next(apiError_1.default.badRequest(`An account with email ${lowercaseEmail} already exists.`));
        return;
    }
    // Create user and send verification email
    try {
        const user = yield (0, user_service_1.createUser)(firstName, lastName, lowercaseEmail, password);
        ...
        if (process.env.NODE_ENV === 'test') {
            ...
            yield user.save();
            yield (0, mail_service_1.emailVerificationLink)(lowercaseEmail, verificationToken);
        }
...
const sendResetPasswordEmail = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!user) {
        next(apiError_1.default.notFound(`No user with email ${lowercaseEmail} is registered.`));
        return;
    }
    // Generate a token for the user for this reset link
    const token = crypto_1.default.randomBytes(32).toString('hex');
    user.resetPasswordToken = token;
    user.resetPasswordTokenExpiryDate = new Date(new Date().getTime() + 60 * 60 * 1000); // Expires in an hour
    yield user.save();
    // Send the email and return an appropriate response
    (0, mail_service_1.emailResetPasswordLink)(lowercaseEmail, token)
        .then(() => res.status(statusCode_1.default.CREATED).send({
        message: `Reset link has been sent to ${lowercaseEmail}`,
    }))
        .catch(() => {
        next(apiError_1.default.internal('Failed to email reset password link.'));
    });
});
...
const resetPassword = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let hashedPassword;
    try {
        hashedPassword = yield (0, bcrypt_1.hash)(password, user_service_1.passwordHashSaltRounds);
    }
    catch (err) {
...
const registerInvite = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
